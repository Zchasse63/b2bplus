...
import { MdReceipt, MdSearch, MdVisibility, MdDownload } from 'react-icons/md';
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div className="flex h-16 w-16 items-center justify-center rounded-xl bg-gradient-to-br from-brand-400 to-brand-600">
              <MdReceipt className="h-8 w-8 text-white" />
            </div>
            <div>
        ...
        <Card extra="p-6">
          <div className="flex items-center justify-between">
            ...
            <div className="flex h-12 w-12 items-center justify-center rounded-lg bg-blue-100 dark:bg-blue-900/30">
              <MdReceipt className="h-6 w-6 text-blue-500" />
            </div>
          </div>
        </Card>
        ...
        <Card extra="p-6">
          <div className="flex items-center justify-between">
            ...
            <div className="flex h-12 w-12 items-center justify-center rounded-lg bg-green-100 dark:bg-green-900/30">
              <MdReceipt className="h-6 w-6 text-green-500" />
            </div>
          </div>
        </Card>
        ...
        <Card extra="p-6">
          <div className="flex items-center justify-between">
            ...
            <div className="flex h-12 w-12 items-center justify-center rounded-lg bg-yellow-100 dark:bg-yellow-900/30">
              <MdReceipt className="h-6 w-6 text-yellow-500" />
            </div>
          </div>
        </Card>
      </div>
      ...
      <Card extra="p-6">
        ...
        {filteredInvoices.length === 0 ? (
          <div className="py-12 text-center">
            <MdReceipt className="mx-auto mb-3 h-16 w-16 text-gray-400" />
            <p className="text-gray-600 dark:text-gray-400">
              {searchQuery ? 'No invoices found matching your search' : 'No invoices yet'}
            </p>
          </div>
        ) : (
          <DataTable data={filteredInvoices} columns={columns} searchable={false} />
        )}
      </Card>
    </div>
  );
